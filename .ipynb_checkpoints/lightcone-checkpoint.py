...
def halo_reader(path_to_files,Total_no_of_Boxes,box_info,jointable,halodir_string):
    ...
    for i in range(len(box_info)):
        ...
        print (file_path)
        data = pd.read_parquet(file_path, columns=['id','pid','upid','scale_of_last_mm','x', 'y', 'z','xoff','vx','vy','vz','c_to_a','b_to_a','vrms','vmax','vpeak','a_x','a_y','a_z','jx','jy','jz','spin','rs','rvir','rvmax','num_prog', 'last_mainleaf_depthfirst_id','m200b','mvir','m200c','desc_id'])
...
#                        'z': pos_z_interp1[sel_lc].values, 'vx':vel_x_ip[sel_lc].values, 'mvir': mvir_interp1[sel_lc].values})
    res = pd.concat([res, newborn[['id','pid','upid','scale_of_last_mm','x', 'y', 'z','xoff','vx','vy','vz','c_to_a','b_to_a','vrms','vmax','vpeak','a_x','a_y','a_z','jx','jy','jz','spin','rs','rvir','rvmax','m200b','mvir','m200c']]], ignore_index=True)
    return res